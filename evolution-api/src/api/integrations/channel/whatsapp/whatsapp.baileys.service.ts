...
import { LocalStorageService } from '@api/integrations/storage/local/services/local.service';
              ...
              if (this.configService.get<LocalStorage>('LOCAL_STORAGE')?.ENABLED) {
                try {
                  ...
                  if (!hasRealMedia) {
                    ...
                    const { fileName: savedFileName, mediaUrl } = await localStorageService.saveMedia(
                      buffer,
                      fileName,
                      mimetype,
                    );
  ...
  private async sendMessageWithTyping<T = proto.IMessage>(
    ...
    try {
      ...
      if (this.configService.get<Database>('DATABASE').SAVE_DATA.NEW_MESSAGE) {
        ...
        // Local Storage handling
        if (isMedia && this.configService.get<LocalStorage>('LOCAL_STORAGE')?.ENABLED) {
          try {
            const message: any = messageRaw;
